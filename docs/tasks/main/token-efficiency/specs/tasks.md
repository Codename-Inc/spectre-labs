# Tasks — Token-Efficient Build Loop (Phase Owner Pattern)
...
| REQ-010 | Remove TASK_COMPLETE → build loopback, adjust max_iterations | plan.md | 2.1 |
...
### Where This Fits
- Modifies the **build stage prompt** (`prompts/build.md`) — the instruction template that runs inside `Stage.run_iteration()` via `AgentRunner`
- Updates **code review prompt** (`prompts/code_review.md`) — removing full doc reads for intentional isolation
- Adjusts `create_default_pipeline()` in `pipeline/loader.py` — transitions and max_iterations
...
- **Artifact flow**: Build artifacts (`phase_task_descriptions`, `files_touched`) flow to code review via `context.update(result.artifacts)` in `executor.py:275` — no executor changes needed
- **Git hooks**: Still work because subagent commits are visible in the repo; `after_stage_hook("build")` collects diff as before

### Key Decisions
- Phase owner = one session per phase (not per task) — reads context once
- Subagents get dynamic prompts (not template files) — matches spectre:execute pattern
- TASK_COMPLETE loopback removed — phase owner handles task iteration internally
- Token tracking deferred to Phase 2 — ship phase owner pattern first
...
1. Task 1.1 - Phase owner build prompt rewrite (no dependencies — core deliverable)
2. Task 2.1 - Pipeline config update (depends on 1.1 — needs to know phase owner signals)
3. Task 2.2 - Code review prompt isolation (depends on 1.1 — needs artifact field names)
4. Task 3.1 - Hooks/artifact verification (depends on 1.1, 2.1, 2.2 — needs all pieces in place)
5. Task 3.2 - E2E dry run test (depends on 3.1 — needs verified system)
6. Task 4.1 - Token tracking from JSONL (independent — can be done anytime after 1.1)

### Parallel Execution

**Wave 1**: 1.1
- Phase owner build prompt — critical path, all other tasks depend on decisions made here
...
- Pipeline config + code review isolation — both depend on 1.1 but independent of each other
...
- Integration verification + E2E test — depend on all prior work being in place
...
- Token tracking — can be done anytime, no dependencies on Waves 1-3
